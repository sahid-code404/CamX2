...
A native ImageReader lease is never allowed to become timestamp-skew or storage-queue state. When an image arrives before its matching capture result, M10 copies only the declared RAW plane extent into detached heap-backed evidence on a dedicated image callback handler and closes the source Image; timestamp pairing then owns only the detached snapshot. Once a pair exists, the asynchronous ingest queue receives a heap-only DetachedRawVideoPair, while canonicalization and hashing run on the ingest worker. This keeps the Camera2 result callback responsive even when a large RAW frame is being copied or the bounded storage queue is full, makes ImageReader.maxImages a short Camera2 delivery bound rather than a hidden buffering mechanism, and directly prevents result-callback skew from exhausting acquired-image slots.
