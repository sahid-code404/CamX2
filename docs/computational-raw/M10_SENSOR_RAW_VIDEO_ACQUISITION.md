...
M10 reserves the canonical ingest queue, the full-frame CXRB spool queue, and detached pairing memory before acquisition. Releasing native `ImageReader` leases is not allowed to create an unaccounted heap buffer elsewhere. The resident admission formula therefore includes both asynchronous full-frame queues, the fixed safety margin, one detached frame that may be in flight from pairing toward ingest, and a bounded number of image-before-result detached frames that may remain in the timestamp-skew map. If the configured resident budget cannot cover both queues plus at least one pending and one in-flight detached RAW frame, recording admission fails closed before Camera2 capture begins.
