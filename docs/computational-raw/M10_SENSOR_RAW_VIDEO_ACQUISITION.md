...
M10 reserves both the canonical ingest queue and detached pairing memory before acquisition. Releasing native `ImageReader` leases is not allowed to create an unaccounted heap buffer. The resident admission formula therefore includes the canonical ingest queue, the fixed safety margin, one detached frame that may be in flight from pairing toward ingest, and a bounded number of image-before-result detached frames that may remain in the timestamp-skew map. If the configured resident budget cannot cover at least one pending plus one in-flight detached RAW frame, recording admission fails closed before Camera2 capture begins.
